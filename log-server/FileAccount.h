...
public:
  ...
  FileAccount(const TCHAR *logDir, const TCHAR *fileName, unsigned char logLevel);
  virtual ~FileAccount();
  ...
  void print(unsigned int processId,
...
private:
  ...
  // Extract log headers and write its to a new log file.
  void writeLogHeader();
  ...
  const StringStorage m_fileName;
